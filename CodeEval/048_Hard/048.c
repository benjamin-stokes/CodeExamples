/* Hungarian Algorithm courtesy of Cyrill Stachniss */
...
void lettercount(char *word, int lettercount[]);
int gcd ( int a, int b );
...
void lettercount(char *word, int lettercount[])
{
    ...
    int i=0;
    wordtmp = strdup(word);
    while (strsep(&wordtmp,letters)) i++;
    lettercount[0] = i-1;
    i = 0;
    wordtmp = strdup(word);
    while (strsep(&wordtmp,vowels)) i++;
    lettercount[1] = i-1;
    lettercount[2] = lettercount[0]-lettercount[1];
}
...
int main(int argc, const char * argv[]) {
    ...
    char *products[20];
    char *token, *linetmp;
    int nNames, nProducts, i=0, j=0;
    int wordcount[2][20][3];
    int  SS[400];
    float SStmp, costmax;
    ...
    while (fgets(line, 1024, file)) {
        ...
        linetmp = strdup (line);
        token = strsep(&linetmp, ";");
        if (strlen(token)<=1 || strlen(linetmp)<=1 )
        {
            printf ("0.00\n");
            fflush(stdout);
            continue;
        ...
        categories[0] = strdup(token);
        categories[1] = strdup(linetmp);
        linetmp = strdup(categories[0]);
        i = 0;
        ...
        nNames = i;
        i = 0;
        linetmp = strdup(categories[1]);
        while (token = strsep(&linetmp, ",")){
            products[i] = strdup(token);
            i++;
        }
        nProducts = i;
        i = 0;
        for (i = 0; i < nNames; i++)
            lettercount(names[i], wordcount[0][i]);
        for (i = 0; i < nProducts; i++)
            lettercount(products[i], wordcount[1][i]);
        for (i = 0; i < nNames; i++)
        {
            for (j = 0; j < nProducts; j++)
            {
                if (wordcount[1][j][0]%2 == 0)
                    SStmp = (float) wordcount[0][i][1]*1.5;
                else SStmp = (float)wordcount[0][i][2];
                if (gcd(wordcount[0][i][0], wordcount[1][j][0]) > 1)
                    SStmp *= 1.5;
                SS[i*nProducts+j]= -(int)(4.*SStmp);
            }
        }
        int** m = array_to_matrix(SS,nNames,nProducts);
        int matrix_size = hungarian_init(&p, m, nNames, nProducts, 0) ;
        ...
        for (i=0; i < nNames; i++)
            for(j=0; j < nProducts; j++)
            {
                costmax += (float)(p.assignment[i][j]*m[i][j]);
            }
        costmax *= -0.25;
        printf ("%.2f\n", costmax );
        hungarian_free(&p);
